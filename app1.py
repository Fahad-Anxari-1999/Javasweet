...
def main():
    ...
    if st.button("Analysis Rush Hours"):
        ...
        # Calculate the average of recent records for each shelf (e.g., the last 10 records)
        recent_records = data
        shelf_averages = recent_records[["Shelf 1", "Shelf 2", "Shelf 3", "Shelf 4"]].mean()

        # Filter rows where the customer count for each shelf is above its average
        above_average_shelves = data[
            (data["Shelf 1"] > shelf_averages["Shelf 1"]) |
            (data["Shelf 2"] > shelf_averages["Shelf 2"]) |
            (data["Shelf 3"] > shelf_averages["Shelf 3"]) |
            (data["Shelf 4"] > shelf_averages["Shelf 4"])
        ]

        # Find the timestamp with the maximum count for each shelf, above their respective averages
        filtered_data = above_average_shelves[["Timestamp", "Shelf 1", "Shelf 1 Above Avg", 
                                       "Shelf 2", "Shelf 2 Above Avg",
                                       "Shelf 3", "Shelf 3 Above Avg", 
                                       "Shelf 4", "Shelf 4 Above Avg"]]

       

        # Save the results to a CSV file
        filtered_data.to_csv("above_average_shelves.csv", index=False, sep=";")

print("Results saved to 'shelf_max_customers.csv'")
